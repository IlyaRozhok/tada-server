...
async function fixAuthIssues() {
  ...
  try {
    ...
    console.log(
      "   1. Update JWT_ACCESS_EXPIRES_IN to '7d' in your environment files"
    );
    console.log(
      "   2. Ensure the frontend stores the token in localStorage after registration"
    );
    console.log(
      "   3. Ensure the frontend sends the token in Authorization header"
    );
    console.log("   4. Check CORS configuration allows credentials");
  } catch (error: any) {
